...
export function TrendingPage() {
  ...
  useEffect(() => {
    const loadArticles = async () => {
      try {
        ...
        // Sort articles by some engagement metric (for now, we'll use date as a proxy)
        const sortedArticles = data.sort((a: NewsArticle, b: NewsArticle) => 
          new Date(b.publishedAt).getTime() - new Date(a.publishedAt).getTime()
        ).slice(0, 15); // Get top 15 trending articles
  ...
  return (
    <div className="container mx-auto px-4 py-8 mt-24">
      <h1 className="text-3xl font-bold mb-8 text-center">Trending News</h1>
